...
export default function SelectionToolbar({
  ...
  onBringFront: () => void;
  onSendBack: () => void;
  // "Opciones de diseño" (rotar por ángulo/90°/espejo, cambiar color,
  // opacidad/brillo/contraste) -- el panel en sí YA NO vive aquí. Pedido
  // explícito: el usuario debe ver la prenda completa junto con el logo
  // MIENTRAS edita -- ni como overlay flotando encima del lienzo, ni
  // empujándolo hacia abajo (las dos formas ya probadas y rechazadas)
  // funcionan para eso. El panel real se renderiza en el sidebar
  // (PersonalizerClient), que nunca toca el tamaño/posición del lienzo --
  // este botón solo dispara/refleja ese estado, controlado por el padre.
  designOptionsOpen: boolean;
  onToggleDesignOptions: () => void;
}) {
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-ui-border bg-white/95 px-3 py-2 shadow-[0_8px_24px_rgba(0,0,0,0.08)] backdrop-blur-sm">
      {element.type === "text" && (
        <>
          ...
          {(["left", "center", "right"] as const).map((align) => (
            <ToolbarIconButton key={align} label={`Alinear ${align}`} onClick={() => onChange(element.id, { align })}>
              <svg
                viewBox="0 0 16 16"
      ...
      {element.type === "logo" && (
        // "Eliminar fondo" (acceso directo de un clic) y "Opciones de
        // diseño" van juntos, uno al lado del otro -- pedido explícito.
        // La sección "Fondo" dentro de DesignOptionsPanel se quitó para
        // no duplicar el mismo control en dos lugares.
        <>
          <button
            type="button"
