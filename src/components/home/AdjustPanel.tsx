...
// Panel temporal de ajuste visual para la tarjeta "Experiencia".
// Eliminar este archivo y su uso en page.tsx cuando el usuario termine de ajustar.
const TARGET_ID = "experiencia-box";

export default function AdjustPanel() {
  const [top, setTop] = useState(-54);
  const [left, setLeft] = useState(-85);
  const [width, setWidth] = useState(722);
  const [height, setHeight] = useState(507);
  ...
  return (
    <div className="fixed top-4 right-4 z-[9999] w-56 rounded-lg border border-ui-border bg-white p-4 shadow-2xl">
      <p className="mb-3 text-xs font-bold text-foreground">Ajuste tarjeta Experiencia (temporal)</p>
